...
class PiecesModel {
  ...
  static getPieceById(id, callback) {
    db.query(
      "SELECT * FROM pieces WHERE ID_Piece = ?",
      [id],
      (err, results) => {
        if (err) {
          callback(err);
  ...
  static updatePiece(id, piece, callback) {
    db.query(
      "UPDATE pieces SET ? WHERE ID_Piece = ?",
      [piece, id],
      (err, results) => {
        if (err) {
          callback(err);
  ...
  static countpiece(callback) {
    db.query("SELECT count(ID_Piece) as pieces from pieces", (err, results) => {
      if (err) {
        callback(err);
      } else {
        callback(null, results);
